...
bool post_jpeg_to_phone(const uint8_t* buf, size_t len) {
  ...
  for (int attempt = 1; attempt <= 3; ++attempt) {
    ...
    int code = http.POST(const_cast<uint8_t*>(buf), len);   // FIX: const -> non-const 변환
    http.end();
...
void handleSnapshot() {
  ...
  server.send_P(200, "image/jpeg", (const char*)fb->buf, fb->len);  // FIX: 타입 캐스팅
  esp_camera_fb_return(fb);
}
...
void setup() {
  ...
  // FIX: setScanResponse(true) 제거 → 스캔 응답 데이터로 대체
  NimBLEAdvertisementData scanData;
  scanData.setName("AI_DOCENT_GLASS");
